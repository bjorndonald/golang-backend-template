...
func (a *AuthHandler) Authenticate(c *gin.Context) {
	...
	_, _, err = helpers.GetDeviceLocation(c)
	if err != nil {
		helpers.ReturnError(c, "Something went wrong", fmt.Errorf("device could not be recognized"), http.StatusInternalServerError)
		return
	}

	// user.CurrentLocation = location
	// user.UserAgents = append(user.UserAgents, userAgent)
...
func (a *AuthHandler) Register(c *gin.Context) {
	...
	if err := a.deps.AgentRepo.Create(userAgent); err != nil {
		helpers.ReturnError(c, "Something went wrong", err, http.StatusInternalServerError)
		return
	}
